...
CSN_Predict_d <- function(d, L, samp, beeta = beeta, sig_e = sig_e, lambda_e = lambda_e, gamma_nd = gamma_nd, 
    mu_u = mu_u, mu_e = mu_e, alpha = alpha, beta = beta, B0 = B0, nu = nu, gamma_inv = gamma_inv, 
    PovLine = 12) {
    ...
    # V0s <- t(rtmg(n=L, M=gamma_inv, r=rep(0,Nds+1), initial=nu_0s+0.1, f = diag(Nds+1), g = -nu_0s,
    # burn.in = 50))
    V0s <- t(rtmvnorm(n = L, mean = rep(0, param$nd + 1), H = gamma_inv, lower = nu_0s, algorithm = "gibbs", 
        burn.in.samples = 500, thinning = 5))
    V0 <- rbind(V0r, V0s)
...
CSN_Predict <- function(samp, beeta, sig_e, sig_u, lambda_u, lambda_e, adjust = TRUE) {
    ...
    B0 <- cbind((lambda_e * sig_e/(1 + lambda_e^2)) * diag(param$Nd - param$nd), b01 * matrix(1, 
        param$Nd - param$nd, param$nd), b02 * rep(1, param$Nd - param$nd))
    gamma_0s <- matrix(rbind(cbind(diag(param$nd) + lambda_e^2 * gamma_nd, -lambda_e * lambda_u * 
        (sig_e/sig_u) * gamma_nd * rep(1, param$nd)), c(-lambda_e * lambda_u * (sig_e/sig_u) * gamma_nd * 
        rep(1, param$nd), 1 + lambda_u^2 * (sig_e/sig_u)^2 * gamma_nd)), param$nd + 1, param$nd + 
        1)
    gamma_inv <- matrix(rbind(cbind(diag(param$nd) - (gamma_nd * lambda_e^2 * sig_u^2/den) * matrix(1, 
        param$nd, param$nd), (lambda_e * lambda_u * sig_e * sig_u * gamma_nd/den) * rep(1, param$nd)), 
        c((lambda_e * lambda_u * sig_e * sig_u * gamma_nd/den) * rep(1, param$nd), (1 + param$nd * 
            gamma_nd * lambda_e^2) * sig_u^2/den)), param$nd + 1, param$nd + 1)
    ...
    nu <- rbind((lambda_e/sig_e) * (diag(param$nd) - gamma_nd * matrix(1, param$nd, param$nd)), (lambda_u/sig_u) * 
        gamma_nd)
    
    # Computes the predictor for all the small areas
    y_pred <- t(sapply(1:param$d, FUN = CSN_Predict_d, L = param$L, beeta = beeta, samp = samp, sig_e = sig_e, 
        lambda_e = lambda_e, gamma_nd = gamma_nd, mu_u = mu_u, mu_e = mu_e, alpha = alpha, beta = beta, 
        B0 = B0, nu = nu, gamma_inv = gamma_inv, PovLine = param$z))
...
CSN_Cond_Predict_d <- function(d, L, samp, beeta, sig_u, lambda_u, sig_e, lambda_e, nu, gamma1, gamma2, 
    Gamma, cj, adjust = TRUE, PovLine = 12) {
    ...
    if (est.Phi.d != 0) {
        u_EBP <- sum(mu_u, as.numeric(gamma1 * sum(res_BP)), dev.Phi.d/est.Phi.d, na.rm = TRUE)
        # sum(mu_u, as.numeric(gamma1*sum(res_BP[pop.ys[,2]==d])),dev.Phi.d/est.Phi.d, na.rm=TRUE)
    } else {
...
CSN_Cond_Predict <- function(samp, beeta, sig_u, lambda_u, sig_e, lambda_e, adjust = TRUE, PovLine = 12) {
    ...
    gamma2 <- sig_u^2 * (1 - 2 * delta_u^2/pi)/(sig_e^2 * (1 - 2 * delta_e^2/pi) + param$nd * sig_u^2 * 
        (1 - 2 * delta_u^2/pi))
    nu <- -rbind((lambda_u/sig_u) * gamma1 * t(rep(1, param$nd)), (lambda_e/sig_e) * (I - gamma1 * 
        J))
    Gamma <- matrix(cbind(c(1 + lambda_u^2 * (1 - param$nd * gamma1), -lambda_u * lambda_e * (sig_e/sig_u) * 
        gamma1 * rep(1, param$nd)), rbind(-lambda_u * lambda_e * (sig_e/sig_u) * gamma1 * t(rep(1, 
        param$nd)), I + lambda_e^2 * gamma1 * J)), param$nd + 1, param$nd + 1)
    R <- diag(1/sqrt(diag(Gamma))) %*% Gamma %*% diag(1/sqrt(diag(Gamma)))
    cj <- c(-lambda_u * sig_e/sqrt(sig_e^2 + param$nd * sig_u^2 + (lambda_u * sig_e)^2), rep(lambda_e * 
        sig_u/sqrt(sig_e^2 + param$nd * sig_u^2 + (lambda_e * sig_u)^2), param$nd))
    
    F_pred <- t(sapply(1:param$d, CSN_Cond_Predict_d, L = param$L, samp = samp, beeta = beeta, sig_u = sig_u, 
        lambda_u = lambda_u, sig_e = sig_e, lambda_e = lambda_e, nu = nu, gamma1 = gamma1, gamma2 = gamma2, 
        Gamma = Gamma, cj = cj, adjust = TRUE, PovLine = param$z))
...
## This function computes the best predictor when errors are SN
SN_Predict_d <- function(d, L, samp, beeta = beeta, sig_e = sig_e, gamma_nd = gamma_nd, mu_u = mu_u, 
    mu_e = mu_e, alpha = alpha, beta = beta, b_u = b_u, b_e = b_e, PovLine = 12) {
    ...
    SN_Predict_d_single <- function(Ndr, Nds, pop.ys, y_ds, xbetar, L, beeta = beeta, sig_e = sig_e, 
        gamma_nd = gamma_nd, mu_u = mu_u, mu_e = mu_e, alpha = alpha, beta = beta, b_u = b_u, b_e = b_e, 
        PovLine = PovLine) {
        
        res_d <- y_ds - (as.vector((pop.ys[, 3:5] %*% beeta)[, 1]) + mu_u + mu_e + b_u * matrix(replicate(L, 
            abs(rnorm(n = 1, mean = 0, sd = 1))), Nds, L, byrow = TRUE) + replicate(L, b_e * abs(rnorm(n = Nds, 
            mean = 0, sd = 1))))
        mu_drs <- xbetar + mu_u + mu_e + b_u * matrix(replicate(L, abs(rnorm(n = 1, mean = 0, sd = 1))), 
            Ndr, L, byrow = TRUE) + b_e * replicate(L, abs(rnorm(n = Ndr, mean = 0, sd = 1))) + matrix(gamma_nd * 
            colSums(res_d), Ndr, L)
        V1 <- replicate(L, rnorm(n = Ndr, mean = 0, sd = sqrt(alpha)) + rnorm(n = 1, mean = 0, sd = sqrt(beta)))
        y_dr <- mu_drs + V1
        y_pred <- rbind(y_dr, matrix(y_ds, Nds, L))
    ...
    f_pred_single <- SN_Predict_d_single(Ndr = Ndr, Nds = Nds, pop.ys = pop.ys, y_ds = y_ds, xbetar = xbetar, 
        L = L, beeta = beeta, sig_e = sig_e, gamma_nd = gamma_nd, mu_u = mu_u, mu_e = mu_e, alpha = alpha, 
        beta = beta, b_u = b_u, b_e = b_e, PovLine = PovLine)
    
    SN_Predict_d_double <- function(Ndr, Nds, pop.ys, y_ds, xbetar, L, beeta = beeta, sig_e = sig_e, 
        gamma_nd = gamma_nd, mu_u = mu_u, mu_e = mu_e, alpha = alpha, beta = beta, b_u = b_u, b_e = b_e, 
        PovLine = PovLine) {
        
        res_d <- y_ds - (as.vector((pop.ys[, 3:5] %*% beeta)[, 1]) + mu_u + mu_e + b_u * abs(rnorm(n = 1, 
            mean = 0, sd = 1)) + b_e * abs(rnorm(n = Nds, mean = 0, sd = 1)))
        mu_drs <- xbetar + mu_u + mu_e + b_u * abs(rnorm(n = 1, mean = 0, sd = 1)) + b_e * abs(rnorm(n = Ndr, 
            mean = 0, sd = 1)) + rep(gamma_nd * sum(res_d), Ndr)
        V1 <- replicate(L, rnorm(n = Ndr, mean = 0, sd = sqrt(alpha)) + rnorm(n = 1, mean = 0, sd = sqrt(beta)))
        y_dr <- mu_drs + V1
        y_pred <- rbind(y_dr, matrix(y_ds, Nds, L))
        F_d <- matrix(data = 0, nrow = 1, ncol = 3)
    ...
    f_pred_double <- rowMeans(replicate(L, SN_Predict_d_double(Ndr = Ndr, Nds = Nds, pop.ys = pop.ys, 
        y_ds = y_ds, xbetar = xbetar, L = L, beeta = beeta, sig_e = sig_e, gamma_nd = gamma_nd, mu_u = mu_u, 
        mu_e = mu_e, alpha = alpha, beta = beta, b_u = b_u, b_e = b_e, PovLine = 12)))
...
SN_Predict <- function(samp, beeta, sig_e, sig_u, lambda_u, lambda_e, adjust = TRUE) {
    ...
    delta_u <- lambda_u/sqrt(1 + lambda_u^2)
    delta_e <- lambda_e/sqrt(1 + lambda_e^2)
    gamma_nd <- sig_u^2 * (1 - delta_u^2)/(sig_e^2 * (1 - delta_e^2) + param$nd * sig_u^2 * (1 - 
        delta_u^2))
    alpha <- sig_e^2 * (1 - delta_e^2)
    beta <- alpha * gamma_nd
    b_u <- delta_u * sig_u
    ...
    # Computes the predictor for all the small areas
    y_pred <- t(sapply(1:param$d, FUN = SN_Predict_d, L = param$L, samp = samp, beeta = beeta, sig_e = sig_e, 
        gamma_nd = gamma_nd, mu_u = mu_u, mu_e = mu_e, alpha = alpha, beta = beta, b_u = b_u, b_e = b_e, 
        PovLine = param$z))
...
NM_Predict_d <- function(d, L, samp, beeta = beeta, sig_e = sig_e, gamma_nd = gamma_nd, alpha = alpha, 
    beta = beta, PovLine = 12) {
...
NM_Predict <- function(samp, beeta, sig_e, sig_u) {
    ...
    # Computes the predictor for all the small areas
    y_pred <- t(sapply(1:param$d, FUN = NM_Predict_d, L = param$L, samp = samp, beeta = beeta, sig_e = sig_e, 
        gamma_nd = gamma_nd, alpha = alpha, beta = beta, PovLine = param$z))
...
ELL_Population <- function(samp, adjust = TRUE, ELL_method = "MOM", resample = "TRUE") {
    ...
    if (ELL_method == "TRAD") {
        ...
        # Computes the predictor for all the small areas
        y_pred <- as.vector(as.matrix(samp[, 3:5]) %*% b_ols) + res_d1 + res_dj1
        # as.matrix(samp[,3:5])%*%beeta + res_d1 + res_dj1 #traditional method
        ...
        x_dev <- x_ss[, 2:3] - cbind(rep(tapply(x_ss[, 2], pop.ss[, 2], mean), each = param$nd), 
            rep(tapply(x_ss[, 3], pop.ss[, 2], mean), each = param$nd))
        SSE_1 <- sum(lm(y_dev ~ 0 + x_dev)$residuals^2)
        SSE_2 <- sum(lm.ss$residuals^2)
        den_1 <- nrow(x_ss) - param$d + ncol(x_dev)
        x_mean <- cbind(tapply(x_ss[, 1], pop.ss[, 2], mean), tapply(x_ss[, 2], pop.ss[, 2], mean), 
            tapply(x_ss[, 3], pop.ss[, 2], mean))
        den_2 <- sum(diag(param$nd * (1 - param$nd * x_mean %*% solve(t(x_ss) %*% x_ss) %*% t(x_mean))))
        
        sig_em <- max(SSE_1/den_1, 1e-09)
        sig_um <- max((SSE_2 - (param$n - 3) * sig_em)/den_2, 1e-09)
...
ELL_RES_Predict <- function(samp, adjust = TRUE) {
    ...
    lm.s <- lm(y_s ~ 0 + x_s)
    b_ols <- as.vector(lm.s$coefficients)
    # cov_b <- vcov(lm.s) beeta <- t(rmvnorm(n=param$L,mean=b_ols,sigma=cov_b))
...
ELL_MOM_Predict <- function(samp, adjust = TRUE) {
    ...
    x_dev <- x_s[, 2:3] - cbind(rep(tapply(x_s[, 2], pop.s[, 2], mean), each = param$nd), rep(tapply(x_s[, 
        3], pop.s[, 2], mean), each = param$nd))
    SSE_1 <- sum(lm(y_dev ~ 0 + x_dev)$residuals^2)
    SSE_2 <- sum(lm.s$residuals^2)
    den_1 <- nrow(x_s) - param$d - ncol(x_dev)
    x_mean <- cbind(tapply(x_s[, 1], pop.s[, 2], mean), tapply(x_s[, 2], pop.s[, 2], mean), tapply(x_s[, 
        3], pop.s[, 2], mean))
    den_2 <- sum(diag(param$nd * (1 - param$nd * x_mean %*% solve(t(x_s) %*% x_s) %*% t(x_mean))))
    
    sig2_em <- max(SSE_1/den_1, 1e-09)
    sig2_um <- max((SSE_2 - (param$n - 3) * sig2_em)/den_2, 1e-09)
...
ELL_Predict <- function(samp, adjust = TRUE) {
    ...
    F_pred <- t(sapply(1:param$d, complex_stats, y1 = y_pred1, y2 = y_pred2, y3 = y_pred3, samp = samp, 
        PovLine = param$z))
